...
The library preserves 36 logo applications and 18 earlier poster studies. Signal is now rounded and upright, with a contained word fragment; the old underlined version is superseded. The commercial task offers six quotation layouts, two per logo, and an invoice example mode. The landing exploration at `homepage.html` has three selectable identity-led layouts. It contains company content, not the task directory. Motion starts automatically and settles; reduced-motion preferences are honored.
...
The current pages, PDF snapshots and exported designs have no author watermark or inspiration shelf. Downloaded PDFs and PNGs are versioned snapshots; regenerate them after visual revisions. `tests/review.js` is the Playwright review/export function for the local workspace. `python tests/check-pdfs.py` checks watermark removal and quoted prices, requires single-page commercial layouts, and rewrites local PDF links to the public site. See `QA.md` for scope and evidence boundaries.
...
Anton and Jost web fonts are bundled with their SIL Open Font License notices in `fonts/`. Brand lettering remains outlined SVG. Photography and source covers retain their existing ownership; this repository does not grant a general reuse license for that artwork.
